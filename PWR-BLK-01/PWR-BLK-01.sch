EESchema Schematic File Version 2
LIBS:device
LIBS:PWR-BLK-01-cache
EELAYER 27 0
EELAYER END
$Descr A 11000 8500
encoding utf-8
Sheet 1 4
Title "One Wire Logger"
Date "16 aug 2014"
Rev "X1"
Comp "dougspcbdesigns.pbworks.com"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 3950 1200 1100 1050
U 517C5D95
F0 "LiPo Battery Charger" 50
F1 "PWR-BLK-01-01.sch" 50
F2 "VUSB" I L 3950 1400 60 
F3 "GND" I L 3950 1950 60 
F4 "VB-3.7" O R 5050 1400 60 
$EndSheet
$Sheet
S 7000 1200 900  1000
U 517C5DAF
F0 "5V Power Supply" 50
F1 "PWR-BLK-01-02.sch" 50
F2 "3.7V" I L 7000 1400 60 
F3 "GND" I L 7000 1950 60 
F4 "+5VOUT" O R 7900 1400 60 
$EndSheet
$Comp
L CONN_1 MTG1
U 1 1 51830500
P 9350 6700
F 0 "MTG1" H 9430 6700 40  0000 L CNN
F 1 "CONN_1" H 9350 6755 30  0001 C CNN
F 2 "MTG-2.5MM" H 10000 6700 60  0000 C CNN
F 3 "~" H 9350 6700 60  0000 C CNN
	1    9350 6700
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 MTG2
U 1 1 5183050F
P 9350 6800
F 0 "MTG2" H 9430 6800 40  0000 L CNN
F 1 "CONN_1" H 9350 6855 30  0001 C CNN
F 2 "MTG-2.5MM" H 10000 6800 60  0000 C CNN
F 3 "~" H 9350 6800 60  0000 C CNN
	1    9350 6800
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 MTG3
U 1 1 5183051E
P 9350 6900
F 0 "MTG3" H 9430 6900 40  0000 L CNN
F 1 "CONN_1" H 9350 6955 30  0001 C CNN
F 2 "MTG-2.5MM" H 10000 6900 60  0000 C CNN
F 3 "~" H 9350 6900 60  0000 C CNN
	1    9350 6900
	1    0    0    -1  
$EndComp
$Comp
L CONN_1 MTG4
U 1 1 5183052D
P 9350 7000
F 0 "MTG4" H 9430 7000 40  0000 L CNN
F 1 "CONN_1" H 9350 7055 30  0001 C CNN
F 2 "MTG-2.5MM" H 10000 7000 60  0000 C CNN
F 3 "~" H 9350 7000 60  0000 C CNN
	1    9350 7000
	1    0    0    -1  
$EndComp
NoConn ~ 9200 6700
NoConn ~ 9200 6800
NoConn ~ 9200 6900
NoConn ~ 9200 7000
$Comp
L CONN_1 FID2
U 1 1 518D20E3
P 9350 6500
F 0 "FID2" H 9430 6500 40  0000 L CNN
F 1 "CONN_1" H 9350 6555 30  0001 C CNN
F 2 "FIDUCIAL" H 9800 6500 60  0000 C CNN
F 3 "~" H 9350 6500 60  0000 C CNN
	1    9350 6500
	1    0    0    -1  
$EndComp
NoConn ~ 9200 6500
$Comp
L CONN_1 FID1
U 1 1 518D21B8
P 9350 6400
F 0 "FID1" H 9430 6400 40  0000 L CNN
F 1 "CONN_1" H 9350 6455 30  0001 C CNN
F 2 "FIDUCIAL" H 9800 6400 60  0000 C CNN
F 3 "~" H 9350 6400 60  0000 C CNN
	1    9350 6400
	1    0    0    -1  
$EndComp
NoConn ~ 9200 6400
$Sheet
S 2400 1200 900  1000
U 53C9C2FF
F0 "FTDI" 50
F1 "PWR-BLK-01-03.sch" 50
F2 "GND" U R 3300 1950 60 
F3 "VUSB" U R 3300 1400 60 
$EndSheet
Wire Wire Line
	3300 1950 3950 1950
Wire Wire Line
	3800 1950 3800 2550
Wire Wire Line
	6800 2550 6800 1950
Wire Wire Line
	6800 1950 7000 1950
Wire Wire Line
	5050 1400 5400 1400
Wire Wire Line
	6650 1400 7000 1400
Wire Wire Line
	3800 2550 9150 2550
Connection ~ 3800 1950
Wire Wire Line
	3950 1400 3300 1400
$Comp
L CONN_2 P1
U 1 1 53C9D166
P 8850 1900
F 0 "P1" V 8800 1900 40  0000 C CNN
F 1 "CONN_2" V 8900 1900 40  0000 C CNN
F 2 "TB2-5MM" H 8850 1700 60  0000 C CNN
F 3 "~" H 8850 1900 60  0000 C CNN
	1    8850 1900
	1    0    0    -1  
$EndComp
Wire Wire Line
	7900 1400 9150 1400
Wire Wire Line
	8250 1400 8250 1800
Wire Wire Line
	8250 1800 8500 1800
Wire Wire Line
	8250 2550 8250 2000
Wire Wire Line
	8250 2000 8500 2000
Connection ~ 6800 2550
$Comp
L SWITCH_INV SW1
U 1 1 53C9D52C
P 5900 1400
F 0 "SW1" H 5700 1550 50  0000 C CNN
F 1 "SW-SPDT" H 5750 1250 50  0000 C CNN
F 2 "SW-SPDT-SLIDE" H 5900 1400 60  0000 C CNN
F 3 "~" H 5900 1400 60  0000 C CNN
	1    5900 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	6400 1300 6650 1300
Wire Wire Line
	6650 1300 6650 1400
$Comp
L CONN_2 P3
U 1 1 53EF7F12
P 9650 1900
F 0 "P3" V 9600 1900 40  0000 C CNN
F 1 "CONN_2" V 9700 1900 40  0000 C CNN
F 2 "PIN_ARRAY_2X1" H 9650 1700 60  0000 C CNN
F 3 "~" H 9650 1900 60  0000 C CNN
	1    9650 1900
	1    0    0    -1  
$EndComp
Wire Wire Line
	9300 1800 9150 1800
Wire Wire Line
	9150 1800 9150 1400
Connection ~ 8250 1400
Wire Wire Line
	9150 2550 9150 2000
Wire Wire Line
	9150 2000 9300 2000
Connection ~ 8250 2550
$EndSCHEMATC
