...
**
** Copyright 2010, USB Design By Example
*/
...
void DisplayChar(BYTE Character) {
    if (Character) LocalBuffer[BufferIndex++] = Character;
    if (BufferIndex >= sizeof(LocalBuffer)) CheckStatus(1, ErrorUARTBufferOverflow);
    }
...
// dprint can be called from multiple threads, so use a mutex to keep output from a single dprint() together
 ...
 void dprintBuffer(BYTE ThreadID, BYTE* BufferPtr, int Count) {
    int i;
    for (i=0; i<Count; i++) {
        if (!(i & 15)) dprint(ThreadID, "\n", 0);
        dprint(ThreadID, "%x ", BufferPtr++);
        }
    }
