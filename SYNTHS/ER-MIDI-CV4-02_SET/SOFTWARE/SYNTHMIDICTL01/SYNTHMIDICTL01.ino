...
// CLK: Output MIDI Clock. The knob can be divided from 24, 12, 6, and 3.
...
void setup() {
 ...
 SPI.setClockDivider(SPI_CLOCK_DIV4) ;// CLK / 4, 16MHz/4)
...
void loop() {
 ...
 if (MIDI.read()) {               // 
   ...
   switch (MIDI.getType()) {
     ...
     case midi::NoteOn://Note On after
       note_on_count ++;
       trigTimer = millis();
