/*
** Main.c module for Chapter8 Example (Stage7)
**
...
// Kitt thread, handles LEDs (see www.knightrideronline.com)
void Kitt(BYTE ThreadID) {
    BYTE PortData = 3;
    BOOL Direction = TRUE;
    BYTE ASCII_Direction;
    vos_wait_semaphore(&DevicesStarted);
    vos_signal_semaphore(&DevicesStarted);
    dprint(ThreadID, "Kitt (ID=%d) has started\n", &ThreadID);

    while (1) {
        CallVOS(ThreadID);
        vos_delay_msecs(Delay/4);
        ThreadRunning(ThreadID);
        // Calculate the new scan pattern
        if (PortData) PortData = Direction ? PortData<<1 : PortData>>1;
        else {
            // Single scan complete, change direction and re-initialize
            Direction = !Direction;
            PortData = Direction ? 0x03 : 0xc0;
            ASCII_Direction = Direction ? 'L' : 'R';
            dprint(ThreadID, "%c", &ASCII_Direction);
            }
        // Now write scan pattern to the GPIO port.
        CallVOS(ThreadID);
        vos_dev_write(hDevice[KittLEDs] ,&PortData, 1, NULL);
        ThreadRunning(ThreadID);
        }
    }
...
// Initialise threads - pass a ThreadID to each thread
        ...
        vos_create_thread(25, SIZEOF_tcb, &Kitt, 1, 8);
