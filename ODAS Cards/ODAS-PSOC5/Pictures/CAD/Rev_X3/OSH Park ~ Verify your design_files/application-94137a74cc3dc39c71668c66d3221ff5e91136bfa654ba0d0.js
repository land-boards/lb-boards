...
(function() {
  ...
  ga('create', 'UA-32110873-1', 'oshpark.com');
  ga('send', 'pageview');

// Specific behaviour for the cart display
(function() {
  then = new Date();
  function profile(p) {
    now = new Date();
    console.log(now-then, p);
    console.profile(p);
    then = now;
  }
  ...
  var Cart = function($cart) {
    ...
    this.initializeCart = function() {
      profile('*** initializeCart');

      this.addEventsToInputs();
      this.enableAjaxHandler();
    };
    ...
    this.enableForm = function() {
      $('#cart-progress').hide()
      profile('enableForm end');
    };

    this.addEventsToInputs = function() {
      var self = this;
      this.cart.on('click', '.js-cart-input-option', function(e) {
          profile('click');
          self.disableForm();
          self.doSubmit(e); 
        });

      this.cart.on('change', '.js-cart-input-qty', function(e) { 
        profile('change'); 
        self.disableForm();
        self.doSubmit(e);
      });
    ...
    this.submitForm = function() {
      profile('submitForm start');
      if (this._formIsSubmitting) { return; }
      this._formIsSubmitting = true;
      this.cart.find('form').trigger('submit.rails');
      profile('submitForm end');
    };

    this.enableAjaxHandler = function() {
      var self = this;
      this.cart.on('ajax:complete', function(_e, xhr, _status) {
        self._formIsSubmitting = false;
        self.enableForm();
        self.cart.html(xhr.responseText);
         profile('ajax:complete');
      });
      profile('enableAjaxHandler end');
    };
...
var UPLOADING_CONTENT = "<i class=\"fa fa-tachometer\"></i> Uploading your design";

var initializeFileSelector = function($el) {
  addChangeHandler($el);
  addSubmitHandler($el);
  hideSubmitButton($el);
};

var addChangeHandler = function($el) {
  $el.find(".file-input").change(function() {
    $el.submit();
  });
};

var hideSubmitButton = function($el) {
  $el.find(".upload-button").hide();
};

var addSubmitHandler = function($el) {
  $el.submit(function() {
    changeToUploading($el);
  });
};

var changeToUploading = function($el) {
  var $box = $el.find('.upload-box');
  $box.addClass('uploading');
  var $content = $box.find('.status-message');
  $content.html(UPLOADING_CONTENT);
};

$(document).ready(function() {
  var $el = $("form#file_selector");

  if ($el.length > 0) {
    initializeFileSelector($el);
  }
});
continueAnyway = function() { 
  $("#order-anyway input[name=\"proceedAnyway\"]").change(function() {
    var checkbox = $(this);
    var continueButton = $("button.continue");

    continueButton.attr("disabled", !checkbox.is(":checked"));
  console.log("order anyway change");
...
var RUNNING_STATES = ["waiting", "running"]

var UploadRefresher = function($el) {
  this.el = $el;

  this.canBeRefreshed = function() {
    return isNotFinished(this.stateElement());
  };

  this.refresh = function() {
    var path = pathIncludingJs();
    var self = this;
  ...
  this.stateElement = function() {
    return this.el.children(".processing-state");
  };

  var pathIncludingJs = function() {
  ...
  var isNotFinished = function($el) {
    for(var i = 0; i < RUNNING_STATES.length; i++) {
      if ($el.hasClass(RUNNING_STATES[i])) {
        return true;
      }
    }
    return false;
